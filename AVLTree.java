...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
         ...
         */
        //if node doesnt exist or tree is empty return null
        if (node == null) {
            return null;
        }

        //if the value you want to delete is smaller than the node, go left
        if (value < node.value) {
            ...
            //if node is = to value check if left or right child and make replacement
            if (node.leftChild == null) {
                return node.rightChild;
            } else if (node.rightChild == null) {
                return node.leftChild;
            }
            // get smallest value in right subtree and make replacement
        ...
        //update the height by checking if left or right heavy and making rotations
        node.height = getMaxHeight(getHeight(node.leftChild), getHeight(node.rightChild)) + 1;
